...
Encog persistence and alternative workflows
------------------------------------------
`NNSim` demonstrates a complementary workflow: it loads three Encog `BasicNetwork` objects from disk (`NNxuse.eg`, `NNyuse.eg`, `NNtuse.eg`) and computes predicted deltas $(\\Delta x, \\Delta y, \\Delta t)$ which it scales into physical units. This shows a hybrid approach where Encog-trained models coexist with the custom `FFNN`/EA setup.
...
-----------------------
This project compiles and runs with a standard Java 8+ toolchain. Example instructions for IntelliJ IDEA (Windows) are below.

1. Prerequisites
   - JDK 8 or later installed and `JAVA_HOME` configured.
   - IntelliJ IDEA Community or Ultimate (or another Java IDE).
   - Place `encog-core-3.4.jar` into the project's `lib/` directory (or add it to the IDE's module dependencies).

2. Importing into IntelliJ
   - Open IntelliJ and choose "Open" → select the project root folder `KheperaCircleNN`.
   - If IntelliJ asks to configure a project SDK, point it to your JDK 8+ installation.
   - Add `lib/encog-core-3.4.jar` to the module dependencies: `File → Project Structure → Modules → Dependencies → + → JARs or directories` and choose the jar. Mark scope as `Compile`.

3. Running examples
   - To run the simulator playback of predictions: set the main class to `Code.RunNetwork` (`src/Code/RunNetwork.java`) and run. Ensure `data/predicted_outputs.csv` exists and contains normalized predictions.
   - To use Encog networks: make sure `NNxuse.eg`, `NNyuse.eg`, and `NNtuse.eg` are in the working directory (or update `NNSim` paths) then run `Code.NNSim`.

4. Building from command line (javac)
   - Compile:

```powershell
javac -cp lib/encog-core-3.4.jar -d out $(Get-ChildItem -Recurse -Filter "*.java" -Name | ForEach-Object {"src/$_"})
```

   - Run (example):

```powershell
java -cp "out;lib/encog-core-3.4.jar" Code.RunNetwork
```

Notes, assumptions and limitations
---------------------------------
- Deterministic EA: random seeds are not centrally controlled; `Random` instances are created locally in operators leading to non-deterministic runs unless seeds are externally managed.
- Fitness interpretation: lower MSE == better; several EA design choices (population size, tournament size, elite count, mutation parameters) are not hard-coded into a single orchestrator in the repository and must be supplied by any harness that drives `EvolutionaryAlgorithm`.
- Physics approximations: `MotionSimulatorPatched` implements a simplified kinematics model appropriate for visualization and algorithm research but not a physics-accurate simulator.
- Logging & testing: the project contains example runners and CSV utilities but no formal unit test suite; extend as needed.

Appendix: mathematical summary
------------------------------
- Chromosome length formula: $L = n_{in}n_{hid} + n_{hid} + n_{hid}n_{out} + n_{out}$.
- Sigmoid activation: $\\sigma(x) = \\frac{1}{1+e^{-x}}$.
- MSE for a single sample: $\\text{MSE}(y,t)=\\frac{1}{m}\\sum_{i=1}^m (t_i-y_i)^2$.

References to source files
--------------------------
- Simulator orchestrator: [src/Code/KheperaSimulator.java](src/Code/KheperaSimulator.java)
- Forward network and chromosome mapping: [src/EvolutionaryRobotics/FFNN.java](src/EvolutionaryRobotics/FFNN.java) and [src/EvolutionaryRobotics/Chromosome.java](src/EvolutionaryRobotics/Chromosome.java)
- Evolutionary operators: [src/EvolutionaryRobotics/EvolutionaryAlgorithm.java](src/EvolutionaryRobotics/EvolutionaryAlgorithm.java)
- CSV loader: [src/DataHandler/DataLoader.java](src/DataHandler/DataLoader.java)
- Prediction runner: [src/Code/RunNetwork.java](src/Code/RunNetwork.java)
- Encog-backed simulator: [src/Code/NNSim.java](src/Code/NNSim.java)

If you want, I can now:
- Add a runnable EA driver (main class) that demonstrates a full evolve→evaluate loop and writes `predicted_outputs.csv`.
- Add unit tests for the genotype→phenotype mapping and `FFNN.fire(...)` numerical correctness.
- Flesh out README and inline Javadoc for the classes above.
